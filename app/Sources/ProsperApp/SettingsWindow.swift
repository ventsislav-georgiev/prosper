...
private struct SettingsRootView: View {
    ...
    private func extensionSettingsPane(registry: ExtensionRegistry, selection: String) -> ExtensionSettingsPane? {
        ...
        // Menu Bar Management: the rich native controls (spacing, hiding +
        // relaunch) ride in as the FOOTER below the manifest's reveal shortcut,
        // so there's ONE Menu Bar Management section.
        case "com.prosper.menubar": footer = AnyView(MenuBarPane())
...
private struct GeneralPane: View {
    ...
    /// The Prosper Remote companion app — the only client for the Remote Terminal
    /// feature, so the settings row links straight to it.
    static let remoteAppStoreURL = URL(string: "https://apps.apple.com/app/prosper-remote/id6782528095")!

    var body: some View {
        NeonScroll {
            PaneTitle(title: "General", subtitle: "Startup, menu bar and clipboard")

            // The four rows every user touches, in one card at the very top: where
            // Prosper lives (login item, menu bar, Dock) and where its panels open.
            // Each row carries its own subtitle since the merged card can't have four
            // footers; the one shared footer covers only the placement picker, whose
            // options need more room than a subtitle gives.
            NeonSection("Essentials",
                        footer: "Panel placement decides where the command runner (⌥Space) and Clipboard History open on a multi-display setup. “Screen under the cursor” follows your pointer like Raycast and Ditto, and still reopens at the exact spot you dragged it to while you stay on that screen. “Last position” always reuses the last spot; “Main screen” always uses the display with the menu bar.") {
                NeonRow("Launch at login",
                        subtitle: "Start Prosper automatically when you log in.") {
                    Toggle("", isOn: Binding(
                ...
                NeonDivider()
                NeonRow("Show menu bar icon",
                        subtitle: "With the icon hidden, Prosper stays reachable via global shortcuts.") {
                    Toggle("", isOn: Binding(
                ...
                NeonDivider()
                NeonRow("Show Dock icon while a window is open",
                        subtitle: "Lets you Cmd-Tab back to Prosper. Off keeps it fully hidden from the Dock and app switcher.") {
                    Toggle("", isOn: Binding(
                ...
                NeonDivider()
                NeonRow("Panel placement",
                        subtitle: "Which display the runner and Clipboard History open on.") {
                    Picker("", selection: $model.runnerPlacement) {
            ...
            // Tail of the pane: the two opt-in, hardware-hungry features. Remote
            // Terminal needs a second device, Local AI needs a downloaded model and
            // the RAM to hold it — neither belongs in the first screenful.
            NeonSection("Remote Terminal",
                        footer: "Serves your terminal sessions to the Prosper Remote iPhone/iPad app over Tailscale. The port binds only to your Tailscale address — never the public internet. Requires Tailscale to be running on both devices.") {
                ...
                NeonDivider()
                NeonRow("Prosper Remote",
                        subtitle: "There's nothing to connect with on the Mac — the sessions are served to the free Prosper Remote app on your iPhone or iPad.") {
                    Button("Get the app") {
                        NSWorkspace.shared.open(Self.remoteAppStoreURL)
                    }
                    .buttonStyle(.neon)
                }
            }

            // One card for both local-model features: same engine, same download,
            // same RAM cost — and each switch hides its whole settings category.
            NeonSection("Local AI",
                        footer: "Both run entirely on this Mac against a local model — nothing is sent to a server. Turning one off hides its whole settings category in the sidebar; the switch stays here, so it's always re-enableable.") {
                NeonRow("Inline autocomplete",
                        subtitle: "System-wide ghost-text completions as you type in any app.") {
                    Toggle("", isOn: Binding(
                ...
                NeonDivider()
                NeonRow("Coding agent",
                        subtitle: "The chat agent and its menu-bar entry, MCP servers, plugins and personas.") {
                    Toggle("", isOn: $model.agentEnabled).labelsHidden()
                }
            }
        }
...
/// NeonScroll, below the declarative shortcut header). Spacing slider, auto-rehide
/// + chevron-style, and the data-loss-safe "relaunch to apply spacing".
/// Reordering is intentionally NOT here — macOS owns icon order (⌘-drag). Prosper
/// once shipped an opt-in engine that drove the bar back to a saved order; it was
/// unreliable across macOS versions and is gone. The pane points at the native
/// gesture instead.
private struct MenuBarPane: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: sz(16)) {
            ...
            NeonSection("Arranging icons",
                        footer: "macOS owns the left-to-right order of the menu bar, and it remembers what you set. Prosper doesn’t override it.") {
                Label("Hold ⌘ and drag an icon in the real menu bar to move it.",
                      systemImage: "hand.draw")
                    .font(Neon.font(.caption)).foregroundStyle(Neon.textSecondary)
                NeonDivider()
                Label("⌘-drag an icon to the LEFT of Prosper’s chevron to hide it, or back to the right to show it again.",
                      systemImage: "eye.slash")
                    .font(Neon.font(.caption)).foregroundStyle(Neon.textSecondary)
            }
        }
        .onAppear { store = Preferences.menuBarStore }
    }
...
func settingsSidebarGroups(registry: ExtensionRegistry?) -> [(String, [SettingsTab])] {
    ...
    // AI Models sits right AFTER Extension Settings: it manages BOTH the inline and
    // agent models (download/load/RAM/custom), so it must show regardless of which
    // feature master-switch is on, but it's a power-user group — the extensions the
    // user installed read first.
...
enum ProsperVerify {
    static func run(registry passedRegistry: ExtensionRegistry) async {
        ...
        // ── Part 1: "AI Models" sits immediately after "Extension Settings". ──
        if let e = order.firstIndex(of: "Extension Settings") {
            let after = order.indices.contains(e + 1) ? order[e + 1] : "<none>"
            print("part1 after-ext-settings: \(after)  (expect: AI Models)")
        } else {
            print("part1 after-ext-settings: EXTENSION SETTINGS MISSING")
        }
