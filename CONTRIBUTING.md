# Contributing to Prosper

Developer documentation: building from source, architecture, extension authoring, and the release pipeline. For product docs see the [README](README.md).

---

## Build & develop

**Requires:** Xcode 16+ / Swift 6, Apple Silicon, macOS 14+. No Ollama, no Python, no model server.

```sh
./scripts/build.sh release      # build
./scripts/bundle.sh release     # package → dist/Prosper.app (ad-hoc, or stable identity if present)
open dist/Prosper.app

# Optional, once per machine: stable signing so privacy grants survive rebuilds
./scripts/make-signing-cert.sh  # see "Code signing & privacy grants" below
```

```sh
./scripts/build.sh debug        # faster builds
.build/debug/ProsperApp         # run from app/ for console logs
```

### Code signing & privacy grants

macOS keys each privacy grant (e.g. Accessibility) to the app's **code-signing identity**, not its file path. An **ad-hoc** build (`codesign --sign -`, the default for `scripts/bundle.sh`) gets a brand-new signature on every rebuild, so an existing grant silently stops matching — the toggle stays **ON** in System Settings but the app is no longer trusted ("toggle on, still not granted"). Two fixes:

- **One-off:** Settings → Context shows a **Reset & re-add** button on the Accessibility row when it isn't trusted — it runs `tccutil reset <service> com.prosper.app` and re-requests, binding a fresh grant to the current binary.
- **Permanent (local dev):** sign with a **stable identity** so the grant survives rebuilds:
  ```bash
  scripts/make-signing-cert.sh        # once per machine — creates "Prosper Self-Signed"
  scripts/build.sh && scripts/bundle.sh   # bundle.sh auto-detects the identity
  ```
  Release builds get this automatically when the `MACOS_SIGN_CERT_P12` / `MACOS_SIGN_CERT_PWD` repo secrets are set (a base64-encoded code-signing `.p12` + its password); CI imports it and exports `PROSPER_SIGN_IDENTITY` for the bundle step. Unset → ad-hoc fallback.

---

## Architecture

```
┌───────────────────────────── Prosper.app ─────────────────────────────┐
│  Swift (AppKit + SwiftUI)                                              │
│   • Menu-bar agent (LSUIElement)                                       │
│   • Global hotkey ⌥L → Translate panel (NSPanel)                       │
│   • Autocomplete: CGEventTap (keys) + Accessibility (caret) + overlay  │
│                                                                        │
│   MLXEngine (in-process, Metal-accelerated) — no IPC, no daemon        │
│   • mlx-swift + ml-explore/mlx-swift-lm (MLXLLM / MLXLMCommon)         │
│   • swift-transformers — HuggingFace Hub download + tokenizer          │
└────────────────────────────────────────────────────────────────────────┘
                          Gemma 4 E2B (6-bit, MLX)
```

| Path | What |
| --- | --- |
| `app/` | SwiftPM package; `ProsperApp` executable |
| `scripts/` | Swift build + `.app` bundling + release |

- **Inference** — [`ml-explore/mlx-swift-lm`](https://github.com/ml-explore/mlx-swift-lm) ships the `gemma4` arch + Gemma 4 E2B/E4B checkpoints (the older `mlx-swift-examples` doesn't).
- **Hub + tokenizer** — [`huggingface/swift-transformers`](https://github.com/huggingface/swift-transformers), via the `#hubDownloader()` / `#huggingFaceTokenizerLoader()` macros.

---

## Writing extensions

A fast, lightweight **Lua extension system** (Neovim-style) — no recompile needed, with a strict, native surface (no webviews).

**One directory per extension:**

```
my-ext/
├── extension.toml   # manifest: id, name, version, api_level, match regex, settings
└── init.lua         # entry script; defines the command handler(s)
```

```toml
# extension.toml
[extension]
id          = "com.example.shout"
name        = "shout"
title       = "Shout"
description = "Uppercase the query."
version     = "1.0.0"
author      = "you"

[extension.host]
min_version = "2.0.0"
api_level   = 1

[extension.entry]
main = "init.lua"

[[contributes.commands]]
id    = "shout.run"
title = "Shout"
mode  = "no-view"        # or "view" (returns a host.ui component tree)
match = "^shout "        # palette routes matching input here
```

```lua
-- init.lua
-- The handler is the global named after the command id with non-alphanumerics
-- replaced by '_': command "shout.run" → shout_run(query). Return a string
-- (or "value\tdetail"), a host.ui.render(...) tree, or nil to decline.
function shout_run(query)
    return (query:gsub("^shout ", "")):upper()
end
```

Useful optional command fields: `keywords` (launcher discovery), `prefix` (e.g. `"sh "` locks the runner into the command as a labelled mode), `launches_window = true` (surface as a row that opens the window on Enter instead of running per keystroke), `requires = ["model"]` (hidden until the local LLM is loaded).

**Runtime** — Lua 5.4 vendored in-process (pure C, no JIT → notarization-clean). Sandboxed (no `io`/`os`/`package`/`require`/file/net) with an instruction budget that aborts runaways. VMs activate lazily on first match — zero cost until used.

**Host API** (`host.*`, strict + time-boxed):

- `host.clipboard.{read,write,history}`
- `host.llm.{complete,translate}` — `translate(text, target [, source])` returns a structured table `{ primary, detected, candidates = { { text=, label=, note= }, … } }` (or nil on empty/failed translation), ready to feed result views
- `host.shell.run`
- `host.window.{frame,set,open,close}` — `frame()`/`set(x,y,w,h)` are focused-window geometry (move/resize) via Accessibility: `frame()` returns the window rect and its screen's visible frame, `set` repositions it. `open(node)` opens a standalone host-rendered window from a declarative `host.ui.*` node (e.g. `host.ui.converter`); `close()` dismisses it (used by form/dialog submit handlers)
- `host.http.{request,get,post}` — outbound HTTP with **built-in retry + exponential backoff** on transient failures (no response / 5xx / 408 / 429); http/https only, time-boxed, response capped at 5 MB. Returns `{ status, body, headers, ok, json }`.
- `host.prefs.{get,set}`
- `host.time()` (wall-clock epoch seconds) · `host.sleep(seconds)` (for backoff; capped)
- `host.notify`
- `host.fs.list_dirs(path)` — immediate subdirectory names of a path (tilde-expanded, hidden entries skipped, sorted); the only filesystem capability, read-only. Powers Quickdirs.
- `host.apps.search(query)` — ranked application matches as `{ { name=, path= }, … }`. Powers the Open launcher.
- `host.files.search{ name=, kind=, ext=, in=, content=, limit= }` — Spotlight (`NSMetadataQuery`) file/folder matches as `{ { name=, path=, display=, isDir=, kind=, size=, modified= }, … }`, ranked by name-match quality blended with **frecency** + recency, scoped to the home folder by default (`in=` overrides; `display` abbreviates it to `~`). Read-only. `host.files.act(id, path)` runs a built-in file operation (`file.open`/`reveal`/`quicklook`/`copyPath`/`copyFile`/`openWith`/`enclosingFolder`/`trash`). Powers the Find Files finder.
- A list item's `actions = { { id=, title=, icon= }, … }` declares the per-row actions the runner surfaces (Open ⏎, Reveal ⌘⏎, Quick Look ⌘Y, and the ⌘K actions panel); reserved `file.*` ids are executed natively.
- `host.json` + declarative UI builders `host.ui.{list,detail,form,grid,converter,loading,render}` → native SwiftUI (no HTML)

`host.ui.loading{ title=, subtitle=, progress= }` renders a native Apple-style loading state — an **infinite** spinner when `progress` is absent, or a **progressive** bar (0…1, with a percentage) when set. The host also shows this spinner automatically while an async view action is in flight, so every async operation gets a polished loading state for free.

`host.ui.converter{ title=, left=, right=, forward=, backward=, mono= }` is a live bidirectional two-pane transform window (open it via `host.window.open`): `forward`/`backward` name global Lua functions (text → text) in your extension — editing the left pane runs `forward` into the right, editing the right runs `backward` into the left. Base64 is the worked example.

Long-running natives are async, bridged to sync Lua with a hard timeout — extensions run off the main thread.

**System vs user** — bundled features ship as *system extensions*: editable and disablable, **resettable to original**, but never uninstallable. Currently **Calc**, **Unit Convert**, **Base64**, **Currency**, **Translate**, **Open**, **Find Files**, **Shell**, **Quicklinks**, **Quickdirs**, **Browser Bookmarks**, and **Window Management** are implemented this way; the conversion engines keep a native fallback, so a disabled/edited extension never loses the feature. Any installed extension whose `match` regex accepts the palette query is dispatched on the off-main async lane — that is how Quicklinks (and user extensions) surface as palette commands. (Temperature conversion stays native — Foundation's affine Fahrenheit constants can't be reproduced byte-for-byte in the Lua port.) Currency is the worked example for the async surface — it fetches daily FX over `host.http` (with retry), caches them via `host.prefs`/`host.time`, and runs on the off-main async lane.

**Everything lives in `~/.config/prosper/extensions`** — one editable directory. On launch, bundled system extensions are *copied* there (missing folders only, never clobbering your edits) and loaded from there, so you can open any extension in your editor and change it live. **Reset to original** re-copies the pristine bundled version over your edits. User extensions install into the same directory and remove freely; system extensions can't be uninstalled, only reset.

**Install from GitHub** — paste a repo/subdir URL into **Settings → Extensions** (`github.com/owner/repo`, `…/tree/<ref>/<subdir>`, `.git`, `git@` forms all parse). Prosper fetches the tarball, validates `extension.toml`, installs.

---

## Releasing & auto-update

Prosper bundles [Sparkle](https://sparkle-project.org); updates ship straight from this repo's **GitHub Releases** (`SUFeedURL` → `…/releases/latest/download/appcast.xml`). On every `v*` tag, [`release.yml`](.github/workflows/release.yml) builds, version-stamps, EdDSA-signs, and publishes the `.zip` + `appcast.xml`, then bumps the Homebrew cask.

**Cut a release:**

```sh
./scripts/release.sh            # auto-bump MINOR from the latest tag, push it
# or: ./scripts/release.sh patch | major | v3.0.0
```

The git tag is the single source of truth. `bundle.sh` stamps the version into `Info.plist` (which carries a `0.0.0` placeholder, never hand-edited); `CFBundleVersion = MAJOR·1e6 + MINOR·1e3 + PATCH` so Sparkle sees a monotonic integer.

**One-time setup** (free, no Apple account):

```sh
# Sparkle signing — generates an EdDSA key, patches SUPublicEDKey
./scripts/setup-sparkle-keys.sh
"$(find .sparkle-tools -name generate_keys)" -x sparkle_private_key.pem
gh secret set SPARKLE_PRIVATE_KEY < sparkle_private_key.pem
rm -f sparkle_private_key.pem        # never commit this

# Homebrew cask auto-publish — PAT with repo write on the tap
gh secret set HOMEBREW_TAP_TOKEN
```

Notes:

- The EdDSA signature is what Sparkle verifies — **independent of notarization**. Notarization ($99/yr) only removes the first-launch Gatekeeper dialog.
- Without `SUPublicEDKey`, **Check for Updates** resolves the feed but rejects unsigned builds (never crashes).
- Without `HOMEBREW_TAP_TOKEN`, releases still publish; only the cask bump is skipped (with a warning).
- The cask sets `auto_updates true`, so brew and Sparkle coexist — install/update via either.
