...
> **Bind an extension command to a chord.** **Settings → Shortcuts → Extension Commands** lists everything the enabled extensions can do and binds a hotkey that runs it outright — no launcher, nothing to type. Extensions that list a fixed set of targets are expanded one row each, so you can bind *one specific* System Settings pane, *one* Quick Toggle (dark mode, hidden files, empty Trash…), or *one* saved script. The defaults an extension ships with (PastePlain's `⌘⌥⇧V`) show up in the same list and are yours to rebind or turn off. Menu commands are the exception — they belong to whichever app is frontmost, so bind a Command Shortcut to **Menu Commands** and search them in the launcher instead.
